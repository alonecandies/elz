    ...
    case ModelProviderName.NEARAI:
      try {
        const config = JSON.parse(
          fs.readFileSync(path.join(os.homedir(), ".nearai/config.json"), "utf8")
        );
        return JSON.stringify(config?.auth);
      } catch (e) {
        elizaLogger.warn(`Error loading NEAR AI config: ${e}`);
      }
      return character.settings?.secrets?.NEARAI_API_KEY || settings.NEARAI_API_KEY;
...
export async function createAgent(character: Character, token: string): Promise<AgentRuntime> {
